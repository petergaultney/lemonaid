...
from lemonaid.brief import render, status, target
...
def _render(dirs: list[Path], place: Path | None, names: list[str], now: float) -> str:
    return render.markdown(target.Target([], dirs, place, names, "x"), now, lambda ref, cwd: "")
...
def test_status_and_now_come_first_then_the_task_then_where_it_is(tmp_path):
    _write(tmp_path / ".z", "brief.md", _BRIEF, 1000)

    out = _render([tmp_path], tmp_path, [], now=1000 + 300)
...
def test_parent_stays_with_the_task_even_right_below_now(tmp_path):
    ...
    out = _render([tmp_path], tmp_path, [], now=1000)
...
def test_the_brief_named_for_the_session_wins(tmp_path):
    ...
    out = _render([tmp_path], tmp_path, ["nosuch", "Pliny"], now=2000)
...
def test_several_unmatched_briefs_are_listed_newest_first_without_their_tasks(tmp_path):
    ...
    out = _render([tmp_path], tmp_path, ["someone-else"], now=2000)
...
def test_state_md_stands_in_for_a_missing_brief(tmp_path):
    ...
    out = _render([tmp_path], tmp_path, [], now=1000 + 7200)
...
def test_nothing_at_all_says_so(tmp_path):
    assert _render([tmp_path], tmp_path, [], now=0).startswith("## Work status")
...
def test_a_brief_above_the_place_is_not_the_places(tmp_path):
    ...
    out = _render([place / "protostellar" / "visits", place], place, [], now=1000)
...
def test_a_lemon_cwd_outside_the_place_has_no_brief(tmp_path):
    ...
    out = _render([lemon_cwd, session_dir], session_dir, [], now=1000)
...
def test_the_first_directory_with_a_brief_wins(tmp_path):
    ...
    assert _render([lemon_cwd, session_dir], session_dir, [], now=1000).startswith("### lemon's")
