...
def test_status_and_now_come_first_and_the_task_goes_below_a_rule(tmp_path):
    _write(tmp_path, "brief.md", _BRIEF, 1000)

    out = status.render(tmp_path, [], now=1000 + 300)
...
def test_the_brief_named_for_the_session_wins(tmp_path):
    _write(tmp_path, "brief-pliny.md", "# A\n\nStatus: working\n", 1000)
    _write(tmp_path, "brief-quokka.md", "# B\n\nStatus: blocked - need a key\n", 2000)

    out = status.render(tmp_path, ["nosuch", "Pliny"], now=2000)
...
def test_several_unmatched_briefs_are_listed_newest_first_without_their_tasks(tmp_path):
    _write(tmp_path, "brief-pliny.md", _BRIEF.replace("make the thing", "older"), 1000)
    _write(tmp_path, "brief-quokka.md", _BRIEF.replace("make the thing", "newer"), 2000)

    out = status.render(tmp_path, ["someone-else"], now=2000)
...
def test_state_md_stands_in_for_a_missing_brief(tmp_path):
    _write(tmp_path, "state.md", "Halfway through the migration.\n", 1000)

    out = status.render(tmp_path, [], now=1000 + 7200)
...
def test_nothing_at_all_says_so(tmp_path):
    assert status.render(tmp_path / ".z", [], now=0).startswith("## Work status")
...
def test_notes_are_found_above_a_subdirectory(tmp_path):
    ...
    assert status.notes_dir(tmp_path / "src" / "pkg") == tmp_path / ".z"
