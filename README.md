...
- **Briefs**: `b` on a session shows where its work stands - the `Status:` and `## Now` of its `.z/brief.md` - in a tmux popup, without switching to it. `lemonaid brief show` prints the same from anywhere
